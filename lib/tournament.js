...
export const buildKnockoutBracket = (standings, pairs, groups) => {
    const pairMap = new Map(pairs.map((pair) => [pair.id, pair]));
    const pairGroupMap = new Map();

    (groups || []).forEach((group) => {
      const groupId = group?.id;
      const pairIds = Array.isArray(group?.pairIds)
        ? group.pairIds
        : Array.isArray(group?.pairs)
          ? group.pairs.map((pair) => pair?.id).filter(Boolean)
          : [];

      pairIds.forEach((pairId) => {
        if (pairId && !pairGroupMap.has(pairId)) {
          pairGroupMap.set(pairId, groupId);
        }
      });
    });

    const seeded = standings.map((standing, index) => ({
      ...standing,
      seedIndex: index,
      groupId: pairGroupMap.get(standing.pairId) || null,
    }));
    ...
    const getRoundName = (matchCount) => roundNameMap.get(matchCount) || `Ronda de ${matchCount}`;
    ...
    const buildFirstRoundSlots = (order) =>
      order.map((seed) => (seed <= seeded.length ? seeded[seed - 1] : null));

    const countFirstRoundConflicts = (slots) => {
      const matches = [];
      let conflictCount = 0;
      ...
      for (let index = 0; index < slots.length; index += 2) {
        ...
        matches.push({ left, right, sameGroup });
      }
    ...
    const slots = buildFirstRoundSlots(seedOrder);
    const mutableSlotIndices = [];

    for (let index = 0; index < slots.length; index += 2) {
      if (slots[index] && slots[index + 1]) {
        mutableSlotIndices.push(index, index + 1);
      }
    }

    let evaluation = countFirstRoundConflicts(slots);
    let attempts = 0;
    const maxAttempts = seeded.length * seeded.length;

    while (evaluation.conflictCount > 0 && attempts < maxAttempts) {
      let bestSwap = null;

      for (let leftIndex = 0; leftIndex < mutableSlotIndices.length; leftIndex += 1) {
        const slotA = mutableSlotIndices[leftIndex];

        for (let rightIndex = leftIndex + 1; rightIndex < mutableSlotIndices.length; rightIndex += 1) {
          const slotB = mutableSlotIndices[rightIndex];

          if (Math.floor(slotA / 2) === Math.floor(slotB / 2)) {
            continue;
          }

          const candidateSlots = slots.slice();
          [candidateSlots[slotA], candidateSlots[slotB]] = [candidateSlots[slotB], candidateSlots[slotA]];
          const candidateEvaluation = countFirstRoundConflicts(candidateSlots);

          if (
            !bestSwap ||
            candidateEvaluation.conflictCount < bestSwap.evaluation.conflictCount ||
            (candidateEvaluation.conflictCount === bestSwap.evaluation.conflictCount &&
              Math.abs(slotA - slotB) < Math.abs(bestSwap.slotA - bestSwap.slotB))
          ) {
            bestSwap = {
              slotA,
              slotB,
      ...
      if (!bestSwap || bestSwap.evaluation.conflictCount >= evaluation.conflictCount) {
        // Caso límite: si no existe una reasignación de slots sin romper la estructura del bracket,
        // se conserva el cruce conflictivo más cercano a la seeding estándar.
        break;
      }

      slots[bestSwap.slotA] = bestSwap.candidateSlots[bestSwap.slotA];
      slots[bestSwap.slotB] = bestSwap.candidateSlots[bestSwap.slotB];
      evaluation = bestSwap.evaluation;
      attempts += 1;
    }

    const firstRoundMatches = evaluation.matches.map((match, index) => {
      const matchId = `r1-m${index + 1}`;
      const pairAId = match.left?.pairId || null;
      const pairBId = match.right?.pairId || null;

      return {
        id: matchId,
        basePairIds: [pairAId, pairBId],
        pairAId,
        pairBId,
        pairALabel: pairAId ? pairMap.get(pairAId)?.name || match.left?.name || 'Pendiente' : 'Bye',
        pairBLabel: pairBId ? pairMap.get(pairBId)?.name || match.right?.name || 'Pendiente' : 'Bye',
        bye: Boolean(!pairAId || !pairBId),
      };
    });
    ...
    const rounds = [
      {
        name: getRoundName(firstRoundMatches.length),
    ...
    while (previousRoundMatches.length > 1) {
      ...
      for (let index = 0; index < previousRoundMatches.length; index += 2) {
        const sourceA = previousRoundMatches[index];
        const sourceB = previousRoundMatches[index + 1];

        nextMatches.push({
          id: `r${roundIndex}-m${index / 2 + 1}`,
  ...
  const resolvedBracket = bracket.map((round) => {
    const resolvedRoundMatches = round.matches.map((match) => {
      ...
      if ((!pairAId || !pairBId) && Array.isArray(match.sourceMatchIds)) {
        ...
        const hasBye = Boolean(match.bye);
        const played = hasBye || Boolean(result && pairAId && pairBId);
        const winnerId = hasBye ? (pairAId || pairBId) : played ? result.winnerId : null;
        ...
        const resolvedMatch = {
          ...
          pairAId,
          pairBId,
          pairALabel: pairAId ? pairMap.get(pairAId)?.name || 'Ganador' : hasBye ? 'Bye' : 'Pendiente',
          pairBLabel: pairBId ? pairMap.get(pairBId)?.name || 'Ganador' : hasBye ? 'Bye' : 'Pendiente',
          played,
          winnerId,
          loserId,
          ready: hasBye || Boolean(pairAId && pairBId),
        };
